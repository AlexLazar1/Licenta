<template>
  <div>
    <div v-if="$page.flash.success && show" class="mb-8 flex items-center justify-between bg-green rounded">
    ...
    <div v-if="Object.keys($page.errors).length > 0 && show" class="mb-8 flex items-center justify-between bg-red-light rounded">
      <div class="flex items-center">
        ...
        <div class="py-4 text-white text-sm font-medium">
          <span v-if="$page.errors[0] == 'isbn_invalid'">The ISBN you have entered could not be found in the GoodReads database!</span>
          <span v-else-if="Object.keys($page.errors).length === 1">There is one form error.</span>
